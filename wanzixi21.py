...
def wanzixi21(classTimeObject, personSheet, wb):
    ws = wb.create_sheet('21级晚自习', 5)
    # 基础地方
    ws.cell(row=4, column=2).value = "姓名"
    # 查找存在的所有日期和人
    sTime = SortedSet()
    for i in classTimeObject:
        if i.classtype == 3 and "21" in i.classname:
            sTime.add(i.time)
    ...
    for tm in sTime:
        ...
        dt = dict()
        for i in classTimeObject:
            if i.time == tm and i.classtype == 3 and "21" in i.classname:
