...
def wanzixi2223(classTimeObject, personSheet, wb):
    ws = wb.create_sheet('22级23级晚自习', 6)
    # 基础地方
    ws.cell(row=4, column=2).value = "姓名"
    # 查找存在的所有日期和人
    sTime = SortedSet()
    for i in classTimeObject:
        if i.classtype == 3 and ("22" in i.classname or "23" in i.classname):
            sTime.add(i.time)
    ...
    for tm in sTime:
        ...
        ws.cell(row=4, column=indexCol).value = "晚自习"

        dt = dict()
        for i in classTimeObject:
            if i.time == tm and i.classtype == 3 and ("22" in i.classname or "23" in i.classname):
